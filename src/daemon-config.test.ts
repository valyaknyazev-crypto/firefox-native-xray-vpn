...
describe('DaemonConfig buildFor', () => {
  ...
  it('handles xhttp/splithttp correctly', () => {
    const server: ProxyServer = {
      id: 'mock-4',
      name: 'test-xhttp',
      type: 'vless',
    ...
    const config = buildFor(server);
    expect(config.outbounds[0].streamSettings.network).toBe('splithttp');
    expect(config.outbounds[0].streamSettings.splithttpSettings.path).toBe('/stream');
  });
});
